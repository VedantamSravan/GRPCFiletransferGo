	...
	if _, err := os.Stat(workingdir); err == nil {
		...
		if status.Code != pb.UploadStatusCode_Ok {
			err = errors.Errorf(
				...
				status.Message)
			return
			//fmt.Println(pb.UploadStatusCode_Ok)
		}
		if(pb.UploadStatusCode_Ok == status.Code){
			err := os.Remove(filename)
			if err != nil {
				fmt.Println("file doesnot exist ",err)
			}
		}
		return
...
func getConfigValues() (string,string,string){
	confContent, err := ioutil.ReadFile("pub_config.yml")
	if err != nil {
